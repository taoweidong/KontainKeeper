...
> ⚠️ **本表是 2026-09-04 的快照，不要拿它当现状看**：H1/M4/M5 的「未修」指的是
> 那个时点的状态，此后架构已整体换成「MQTT Broker + 无状态桥接 + SQLAlchemy async」，
> **这些项多已在 §0 处置完**（H1 → B0/B1 异步化；M4 → A2/A3 无状态桥接；M5 → P1-5
> 登录限流，B3 又补了 IP 维度）。**判断现状一律以 §0 为准**，本表只用于追溯首版结论。
