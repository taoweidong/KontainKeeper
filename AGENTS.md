...
- **Windows 开发机兼容**：`resource` 模块不存在，`agent/kk_agent/collector.py` 已做 try/except 导入；采集逻辑全部经 `fs_root`（KK_FS_ROOT）注入，测试用 `tests/conftest.py::make_fake_fs` 伪造 /proc 树——采集相关测试不要直接读真实 `/proc`。
...
- 心跳间隔下限 1s（`agent/kk_agent/config.load`），集成测试依赖 1s 间隔在数秒内积累多个序列点；别把下限调回去。
